...
async function main() {
  ...
  // Seed built-in Super Admin role — always update permissions to include ALL
  ...
  // Seed default Admin role (can be edited/deleted by Super Admin)
  await prisma.role.upsert({
    where: { name: "Admin" },
    update: {},
    create: {
  ...
  // Seed default Tech Admin role (can be edited/deleted by Super Admin)
  await prisma.role.upsert({
  ...
  console.log("Seeded default roles (Super Admin, Admin, Tech Admin)");
