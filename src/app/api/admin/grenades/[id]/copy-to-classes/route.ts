...
/**
 * POST /api/admin/grenades/:id/copy-to-classes
 * Copies a single grenade to one or more target classes. Mirrors /api/admin/weapons/:id/copy-to-classes semantics.
 */
export async function POST(
  ...
  const src = await prisma.grenade.findUnique({ where: { id } });
  if (!src) {
  ...
  await prisma.$transaction(async (tx) => {
    for (const targetClass of classes) {
      const existing = await tx.grenade.findFirst({
