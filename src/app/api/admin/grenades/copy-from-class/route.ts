...
/**
 * POST /api/admin/grenades/copy-from-class
 * Copies all grenades from sourceClass into targetClass. Mirrors /api/admin/weapons/copy-from-class semantics.
 */
export async function POST(request: Request) {
  ...
  const source = await prisma.grenade.findMany({ where: { class: sourceClass } });
  ...
  await prisma.$transaction(async (tx) => {
    if (deleteExisting) {
      const res = await tx.grenade.deleteMany({ where: { class: targetClass } });
      deleted = res.count;
    }
    ...
    for (const src of source) {
      const existing = deleteExisting
        ? null
        : await tx.grenade.findFirst({ where: { guid: src.guid, class: targetClass } });
